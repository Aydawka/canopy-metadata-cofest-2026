...
Research data is only as reusable as its description. Before a dataset can be shared and found, someone has to describe it with structured metadata — and doing that by hand is slow, error-prone, and intimidating (the "blank page" problem). It is one of the biggest reasons otherwise valuable datasets never get described well enough to be discovered and reused.

[Canopy](https://github.com/canopy-datahub) addresses the sharing side. It is open-source, generalist **repository infrastructure**: a researcher registers a *study* and submits both its **data files** and the **metadata** that describes them, and Canopy makes the result findable and downloadable by others according to an access policy. It is derived from the [NIH RADx Data Hub](https://radxdatahub.nih.gov/). Every submission is organized around a study, and Canopy ships with a **generic study-metadata template** that every study fills in (title, investigators, design, and so on) — the "Canopy Study template" referenced throughout this document.

The metadata itself is built on [**CEDAR**](https://cedar.metadatacenter.org/) (the Center for Expanded Data Annotation and Retrieval). Two ideas matter: a **template** is a reusable blueprint defining *what* metadata to capture — its fields, their types, and which values are allowed (for example, a "country" field must come from a controlled list, or a date must be a real date); an **instance** is a single record *filled in* against a template. A template is the empty form; an instance is the completed one.

Put together, the **common workflow** is: a researcher has data they want to share; before submitting it they must describe it — locate the data, extract the relevant facts, and define the structure that captures them — then submit the study and its files to Canopy, leaning on CEDAR to describe everything properly. The friction lives in that description step: building templates and filling in valid instances, mostly by hand.

**The overall goal of this project is an AI-assisted workflow that does all of this** — reading a researcher's ordinary files (spreadsheets, data dictionaries, protocols, papers) and producing standards-compliant CEDAR metadata and a registered Canopy study, with a person reviewing along the way.
